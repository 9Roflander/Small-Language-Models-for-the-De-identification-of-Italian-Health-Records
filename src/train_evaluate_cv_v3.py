"""
Full 5-fold cross-validation with the v3 setup.

For each fold:
- Train on: 64 gold (5x oversampled = 320) + 624 synth_with_names + 254 synth_noname + 80 CRF
- Test on: 16 held-out gold using paper's deterministic placeholder-count metric

Reports per-fold + aggregated mean ± std-dev for macro and per-category F1.
"""
...
RESULTS_PATH = "./cv_v3_results.json"
SEED = 42
...
def load_synth(path, label):
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    rows = [{"input": r["text"], "output": r["redacted_text"]}
            for r in data if r.get("text") and r.get("redacted_text")]
...
def get_train_dataset(train_gold):
    ds_named = load_synth(SYNTH_WITH_NAMES, "synth_with_names")
    ds_noname = load_synth(SYNTH_NONAME, "synth_noname")
...
def run_cv():
    ...
    all_results = {"folds": [], "config": {"seed": SEED, "synth_named": SYNTH_WITH_NAMES,
                                            "synth_noname": SYNTH_NONAME, "gold": GOLD_PATH}}
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(gold_arr)):
        ...
        fold_out_dir = f"{CV_OUTPUT_BASE}/fold_{fold_num}"
        args = SFTConfig(
        ...
        print(f"  Fold time: train={t_train/60:.1f}min, total={t_total/60:.1f}min")

        all_results["folds"].append({
            "fold": fold_num,
            "train_size": len(train_gold),
            "test_size": len(test_gold),
            "scores": fold_scores,
            "macro_f1": macro,
        })
    ...
    print(f"\n{'Category':<22}{'P (μ±σ)':>20}{'R (μ±σ)':>20}{'F1 (μ±σ)':>20}")
    print("-" * 80)
    agg = {}
    for t in VALID_TYPES:
    ...
    all_results["aggregate"] = {
        "per_category": agg,
        "macro_f1_mean": macro_m,
        "macro_f1_std": macro_s,
        "fold_macros": macros,
    }
    with open(RESULTS_PATH, 'w', encoding='utf-8') as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
